...
forvalues year = 2000(1)2019 {
	insheet using "$input\daily_42401_`year'.csv", comma names clear
	
	* Use 1-hour 2010 SO2 standard measurements *
	keep if pollutantstandard == "SO2 1-hour 2010"
	...
	* Drop unused variables *
	...
	compress
	save "$inter\42401_daily_`year'", replace
}
*
...
clear all
forvalues year = 2000(1)2019 {
	append using "$inter\42401_daily_`year'"
}
*
...
compress
save "$inter\SO2_daily_00_19", replace
...
use "$inter\SO2_daily_00_19", clear
...
ren stmaxvalue stmaxvalue_SO2
label var stmaxvalue_SO2 "Daily Avg. Maximum SO2 (parts per billion)"

compress
save "$output\SO2_monthly_state_avg_00_19", replace
