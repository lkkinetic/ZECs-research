...
forvalues year = 2000(1)2019 {
	insheet using "$input\daily_88101_`year'.csv", comma names clear
	...
	* Drop unused variables *
	...
	compress
	save "$inter\88101_daily_`year'", replace
}
*
...
/*
Append annual PM2.5 datasets
*/
clear all
forvalues year = 2000(1)2019 {
	append using "$inter\88101_daily_`year'"
}
*
...
compress
save "$inter\PM25_daily_00_19", replace
...
use "$inter\PM25_daily_00_19", clear
...
collapse (mean) stmaxvalue aqi, by(local_date weekday climreg year month fipst state county_fips)
* Then, to state-level *
collapse (mean) stmaxvalue aqi, by(local_date weekday climreg year month fipst state)
* Then, by month *
collapse (mean) stmaxvalue aqi, by(climreg year month fipst state)

ren stmaxvalue stmaxvalue_PM
label var stmaxvalue_PM "Daily Avg. Maximum PM2.5 (micrograms/cubic meter)"

compress
save "$output\PM25_monthly_state_avg_00_19", replace
